...
impl MyModuleSpec for MyModule {
    fn get_data_path(&mut self) -> String {
        // `self.ctx.data_path` is the application's data path
...
impl Storage {
    fn data_file_path(&self) -> std::path::PathBuf {
        std::path::PathBuf::from(&self.ctx.data_path).join("storage.txt")
    }
}
...
impl StorageSpec for Storage {
    ...
    fn write_data(&mut self, value: &str) -> Boolean {
        std::fs::write(self.data_file_path(), value).is_ok()
    }

    fn read_data(&mut self) -> Nullable<String> {
        match std::fs::read_to_string(self.data_file_path()) {
