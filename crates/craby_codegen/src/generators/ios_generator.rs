...
use craby_common::{
    constants::ios_base_path,
    utils::string::{flat_case, pascal_case},
};
use indoc::formatdoc;
...
impl IosTemplate {
    ...
    /// Generates the Swift Package Manager manifest used by React Native 0.87+ autolinking.
    fn package_swift(&self, ctx: &CodegenContext) -> String {
        let package_name = pascal_case(&ctx.project_name);
        let rust_target_name = format!("{package_name}Rust");
        let xcframework_path = format!(
            "ios/framework/lib{}.xcframework",
            flat_case(&ctx.project_name)
        );
        let objc_provider = ObjCProviderName::from(&ctx.project_name);
        let mut sources = vec![
            format!("ios/{objc_provider}.mm"),
            "ios/src/ffi.rs.cc".to_string(),
        ];
        sources.extend(
            ctx.schemas
                .iter()
        ...
        formatdoc! {
            ...
            // React Native resolves this package through build/generated/autolinking/libs/{package_name}.
            ...
            let package = Package(
                ...
                targets: [
                    .binaryTarget(
                        name: "{rust_target_name}",
                        path: "{xcframework_path}"
                    ),
                    .target(
                        name: "{package_name}",
                        dependencies: [
                            .target(name: "{rust_target_name}"),
...
impl Template for IosTemplate {
    ...
    fn render(
        ...
        let res = match file_type {
            ...
            IosFileType::PackageSwift => {
                vec![TemplateResult {
                    path: ctx.root.join("Package.swift"),
                    content: self.package_swift(ctx),
                    overwrite: true,
                }]
            }
        };
...
impl Generator<IosTemplate> for IosGenerator {
    ...
    fn generate(&self, ctx: &CodegenContext) -> Result<Vec<TemplateResult>, anyhow::Error> {
        ...
        let files = [
            template.render(ctx, &IosFileType::ModuleProvider)?,
            template.render(ctx, &IosFileType::PackageSwift)?,
        ]
        .into_iter()
        .flatten()
        .collect();
