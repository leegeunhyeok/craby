...
public:
  ...
  void emit(uintptr_t id, rust::Str name) const {
    ...
    os_log(OS_LOG_DEFAULT, "[C++] emit: %s (Module: %lu)", stdName.c_str(), id);
    
    for (const auto& [id, delegate] : delegates_) {
      delegate(stdName);
      os_log(OS_LOG_DEFAULT, "[C++] call delegate");
    }
  }
  ...
  void registerDelegate(uintptr_t id, Delegate delegate) const {
    ...
    os_log(OS_LOG_DEFAULT, "[C++] registerDelegate: %lu", id);
  }
